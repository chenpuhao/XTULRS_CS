...
public partial class Admin : Form
{
    ...
    private void AddUserData(String json)
    {
        ...
        var userDataList = JsonSerializer.Deserialize<List<UserInfo>>(json, options);
        if (userDataList != null)
        {
            foreach (var userData in userDataList)
            {
                ...
                if (seats != "" && seats != "none")
                {
                    ...
                    var seatDataList = JsonSerializer.Deserialize<List<User.Reservation>>(seats, seatOptions);
    ...
    private void changeUsername_Click(object sender, EventArgs e)
    {
        if (userDataBox.SelectedCells.Count > 0)
        {
            int selectedRowIndex = userDataBox.SelectedCells[0].RowIndex;
            string email = userDataBox.Rows[selectedRowIndex].Cells[1].Value.ToString();
            string name = InputBox.Show("请输入新的用户名", "修改用户名");
    ...
    private void changePassword_Click(object sender, EventArgs e)
    {
        if (userDataBox.SelectedCells.Count > 0)
        {
            int selectedRowIndex = userDataBox.SelectedCells[0].RowIndex;
            string email = userDataBox.Rows[selectedRowIndex].Cells[1].Value.ToString();
            string password = InputBox.Show("请输入新的密码", "修改密码");
    ...
    private void clearSeats_Click(object sender, EventArgs e)
    {
        if (userDataBox.SelectedCells.Count > 0)
        {
            int selectedRowIndex = userDataBox.SelectedCells[0].RowIndex;
    ...
    private void AddSeatData(string seatsJson)
    {
        ...
        var seatsList = JsonSerializer.Deserialize<List<User.Seats>>(seatsJson, options);
        if (seatsList != null)
        {
    ...
    private void AddResData(string resJson)
    {
        ...
        var resList = JsonSerializer.Deserialize<List<Statistics>>(resJson, options);
        if (resList != null)
        {
    ...
    {
        using (var addSeat = new addSeatForm())
        {
            addSeat.ShowDialog();
            string seatsJson = Marshal.PtrToStringAnsi(Dll.returnAllSeats(Program.SeatHead));
            if (!string.IsNullOrEmpty(seatsJson))
    ...
    private void exportSeats_Click(object sender, EventArgs e)
    {
    ...
    private void button2_Click(object sender, EventArgs e)
    {
        ...
        if (roomNum.SelectedItem != null)
        {
            ...
            var allSeats = JsonSerializer.Deserialize<List<User.Seats>>(roomsJson, options);
            var filteredSeats = JsonSerializer.Deserialize<List<User.Seats>>(seatsJson, options);
    ...
    private void button_Click(object sender, EventArgs e)
    {
        ...
        if (roomRes.SelectedItem != null)
        {
            int selectedRoom = (int)roomRes.SelectedItem;
            ...
            var allSeats = JsonSerializer.Deserialize<List<Statistics>>(resJson, options);
            var filteredSeats = JsonSerializer.Deserialize<List<Statistics>>(seatsJson, options);
        ...
        if (seatRes.SelectedItem != null)
        {
            int selectedSeat = (int)seatRes.SelectedItem;
        ...
        if (userRes.SelectedItem != null)
        {
            string selectedUser = (string)userRes.SelectedItem;
        ...
        if (checkBox1.Checked)
        {
            ...
            var filteredSeats = JsonSerializer.Deserialize<List<Statistics>>(filteredJson, options);
    ...
    {
        roomRes.SelectedItem = null;
        seatRes.SelectedItem = null;
        userRes.SelectedItem = null;
        dateTimeStart.Value = DateTime.Now;
    ...
    private void removeSat_Click(object sender, EventArgs e)
    {
    ...
    private void clearSat_Click(object sender, EventArgs e)
    {
